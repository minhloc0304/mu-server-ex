...
// GS-N	2.00.05	JPN	0x00591EB0	-	Completed
//////////////////////////////////////////////////////////////////////
...
void CCashItemPeriodSystem::Initialize()
{
	...
	if (g_bRunningPeriodCheckThread == FALSE )
	{
		...
		{
			CLog.LogAdd("CreateThread() failed with error %d", GetLastError());
			return;
		}
	}
...
void CCashItemPeriodSystem::GDReqPeriodItemInsert(LPOBJ lpObj, int iItemCode, int iItemEffectDate)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x03, sizeof(PMSG_REQ_PERIODITEM_INSERT));

	cDBSMng.Send((char *)&pMsg, sizeof(PMSG_REQ_PERIODITEM_INSERT));

	CLog.LogAdd("[CashShop][PeriodItemInsert Req] - ID : %s, Name : %s, ItemCode : %d, Effect1 : %d, Effect2 : %d, UseTime : %d, ExpireDate : %s",
...
void CCashItemPeriodSystem::GDReqPeriodItemList(LPOBJ lpObj)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x05, sizeof(PMSG_REQ_PERIODITEM_LIST));
...
void CCashItemPeriodSystem::GDReqPeriodItemInquiry(LPOBJ lpObj)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x01, sizeof(PMSG_REQ_PERIODITEM_INQUIRY));
...
void CCashItemPeriodSystem::GDReqPeriodItemUpdate(LPOBJ lpObj)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x07, sizeof(PMSG_REQ_PERIODITEM_UPDATE));
...
void CCashItemPeriodSystem::DGAnsPeriodItemInsert(PMSG_ANS_PERIODITEM_INSERT * aRecv)
{
	...
	if ( btResult == 1 )
	{
		...
		CLog.LogAdd("[CashShop][PeriodItemInsert Ans] Success - ID : %s, Name : %s, ItemCode : %d, Effect1 : %d, Effect2 : %d, UseTime : %d",
			lpObj->AccountID, lpObj->Name, aRecv->iItemCode, aRecv->btItemEffectType1, aRecv->btItemEffectType2, aRecv->iItemPeriodDate);
	}
	else
	{
		CLog.LogAdd("[CashShop][PeriodItemInserver Ans] Fail - ID : %s, Name : %s, ItemCode : %d, Effect1 : %d, Effect2 : %d, UseTime : %d, Result : %d",
...
void CCashItemPeriodSystem::DGAnsPeriodItemList(PMSG_ANS_PERIODITEM_LIST* aRecv)
{
	...
	if ( iResult == -1 )
	{
		CLog.LogAdd("[CashShop][PeriodItemSelect Ans] Failed - Item over 1 item ID : %s, Name : %s",
			lpObj->AccountID, lpObj->Name);
	}
	else if ( iResult == -2 )
	{
		CLog.LogAdd("[CashShop][PeriodItemSelect Ans] Failed - Item usedtime is over ID : %s, Name : %s",
			lpObj->AccountID, lpObj->Name);
	}
	...
	if ( iItemLeftUseTime > 0 )
	{
		...
		CLog.LogAdd("[CashShop][PeriodItemList Ans] Success - ID : %s, Name : %s, ItemCode : %d, Effect1 : %d, Effect2 : %d, UseTime : %d",
			lpObj->AccountID, lpObj->Name, aRecv->iItemCode, aRecv->btEffectType1, aRecv->btEffectType2, iItemLeftUseTime);
	}
	else
	{
		CLog.LogAdd("[CashShop][PeriodItemList Ans] Fail - ID : %s, Name : %s, ItemCode : %d, Effect1 : %d, Effect2 : %d, UseTime : %d",
			lpObj->AccountID, lpObj->Name, aRecv->iItemCode, aRecv->btEffectType1, aRecv->btEffectType2, iItemLeftUseTime);
	}
}
...
void CCashItemPeriodSystem::DGAnsPeriodItemUpdate(PMSG_ANS_PERIODITEM_UPDATE *aRecv)
{
	...
	switch ( aRecv->btResult )
	{
		case 3:
			...
			{		
				CLog.LogAdd("[CashShop][PeriodItemUpdate Ans] Remove Item Effect - ID : %s, Name : %s, ItemCode : %d, Result : %d",
					lpObj->AccountID, lpObj->Name, aRecv->iItemCode, aRecv->btResult);
			}
	}

	CLog.LogAdd("[CashShop][PeriodItemUpdate Ans] - ID : %s, Name : %s, ItemCode : %d, Result %d",
		lpObj->AccountID, lpObj->Name, aRecv->iItemCode, aRecv->btResult);
}
...
void CCashItemPeriodSystem::GDReqPeriodItemDelete(LPOBJ lpObj, LPSTR pchCharacterName)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x09, sizeof(PMSG_REQ_PERIODITEM_DELETE));
...
void CCashItemPeriodSystem::GCSendPeriodItemInserResult(LPOBJ lpObj, BYTE btResult)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD0, 0x03, sizeof(PMSG_ANS_PERIODITEM_INSERT_RESULT));

	gSendProto.DataSend(lpObj->m_Index, (LPBYTE)&pMsg,	sizeof(PMSG_ANS_PERIODITEM_INSERT_RESULT));
}
