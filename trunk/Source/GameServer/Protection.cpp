...
bool UnkownC00::Unknown193()
{
	...
	if(!Unknown172(Unk291))
	{
		Sleep(1000);
		// ------
		MessageBox(NULL,"                              Contact Service Provider\n          You don't have license to use Zodea GameServer","Error() Policy License",MB_OK);
		// ------
		ExitProcess(0);
		return true;
	}
	...
	else if( Unk189 != Unk120 )
	{
		Sleep(1000);
		// ------
		MessageBox(NULL,"                              Contact Service Provider\n          In Order to Register Your License & To Get The Serial.","Error() Zodea License",MB_OK);
		// ------
		ExitProcess(0);
		return true;
	}
