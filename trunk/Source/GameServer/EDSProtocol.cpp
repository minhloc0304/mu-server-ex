...
// Description: MU Union System Protocol Structures & Functions			//
...
void ExDataServerLoginResult(SDHP_RESULT * lpMsg)
{
	...
	{
		CLog.LogAdd("ExDB Server Login Fail: %d", lpMsg->Result);
...
void GDGuildCreateRecv(SDHP_GUILDCREATE_RESULT * lpMsg)
{
	...
	if (lpMsg->Result == 0 )
	{
		...
		C1HeadSet((LPBYTE)&pResult, 0x56, sizeof(pResult));
		pResult.Result = 0;
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( aIndex >= 0 && lpNode != NULL )
		{
			...
			CLog.LogAdd("[U.System] Guild is Created - Guild : %s / [%s][%s]",
				szGuildName, gObj[aIndex].AccountID, gObj[aIndex].Name);

			CLog.LogAdd(lMsg.Get(MSGGET(1, 177)), lpNode->WarState, lpNode->WarDeclareState, szGuildName);
	...
	if ( lpMsg->Result == 1 || lpMsg->Result == 4)
	{
		...
		for ( int n=0;n<MAX_USER_GUILD;n++)
		{
			...
			if ( lpGuild->Use[n] > 0 && iGuildMemberIndex != -1 )
			{
				...
				CLog.LogAdd(lMsg.Get(MSGGET(1, 178)), gObj[n].Name);
			}
		}
		...
		CLog.LogAdd(lMsg.Get(MSGGET(1, 179)), szGuildName, szMaster);
	}
}
...
void GDGuildMemberDelResult(SDHP_GUILDMEMBERDEL_RESULT * lpMsg)
{
	...
	if ( lpMsg->Flag == 1 )
	{
		...
		if ( gObjIsConnected(aIndex) == TRUE )
		{
			...
			//{
				...
				{
					GCResultSend(aIndex, 0x53, lpMsg->Result);
					CLog.LogAdd(lMsg.Get(MSGGET(1, 180)), gObj[aIndex].Name);
				}
				else
				{
		...
		{
			CLog.LogAdd(lMsg.Get(MSGGET(1, 181)), gObj[aIndex].Name);
		}
	}
				...
				if ( gObj[iMember].GuildNumber > 0 )
				{
					if ( gObj[iMember].Name[0] == szMember[0] && gObj[iMember].Name[1] == szMember[1] )
					{
						if ( strcmp(gObj[iMember].Name, szMember) == 0 )
						{
							...
							CLog.LogAdd(lMsg.Get(MSGGET(1, 182)), aIndex, gObj[iMember].Name);
							break;
						}
					}
				}
	...
	{
		CLog.LogAdd(lMsg.Get(MSGGET(1, 183)), szGuildName, lpMsg->Result);
	}
}
		...
		if ( gObj[n].Connected > PLAYER_LOGGED )
		{
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					g_CastleSiege.NotifySelfCsJoinSide(n); // #Erro não sei se ta certo aqui
#endif
					...
					if ( gObj[n].lpGuild != NULL )
					{
						...
						CLog.LogAdd("[U.System][Guild Status][Info] Guild : %s / [%s][%s] / %d",
							szGuildName, gObj[n].AccountID, szName, gObj[n].GuildStatus);

						CLog.LogAdd(lMsg.Get(MSGGET(1, 184)), gObj[n].AccountID, szName, szGuildName);
						...
						gSendProto.DataSend(n, (LPBYTE)GuildInfoBuf, GuildInfoOfs);
...
#endif
						CLog.LogAdd(lMsg.Get(MSGGET(1, 185)));
					}
...
void DGGuildScoreUpdate(LPSTR guildname, int score)
{
	...
	CLog.LogAdd(lMsg.Get(MSGGET(1, 186)), guildname, score);
}
...
void GDGuildScoreUpdateRecv(SDHP_GUILDSCOREUPDATE * lpMsg)
{
	...
	CLog.LogAdd(lMsg.Get(MSGGET(1, 187)), lpMsg->GuildName, lpMsg->Score);
}
...
void DGGuildNoticeRecv(SDHP_GUILDNOTICE * lpMsg)
{
	...
	CLog.LogAdd(lMsg.Get(MSGGET(1, 188)), lpMsg->GuildName, lpMsg->szGuildNotice);
			...
			if ( lpGuildInfo->Use[i] != FALSE )
			{
				...
				if ( iUserIndex >= 0 )
				{
					if ( lpGuildInfo->Names[i][0] == gObj[iUserIndex].Name[0] )
					{
						...
						{
							gSendProto.DataSend(iUserIndex, (LPBYTE)&pMsg, pMsg.h.size);
						}
					}
				}
			}
...
void DGGuildAnsAssignStatus(EXSDHP_GUILD_ASSIGN_STATUS_RESULT * lpMsg)
{
	if ( lpMsg->btFlag == 1 )
	{
		...
		gSendProto.DataSend(lpMsg->wUserIndex, (LPBYTE)&pMsg, sizeof(pMsg));
		...
		CLog.LogAdd("[U.System][Guild Status][Assign] Guild : ( %s ) Member : ( %s ) Status : ( %d )",
...
void DGRelationShipAnsJoin(EXSDHP_RELATIONSHIP_JOIN_RESULT * lpMsg)
{
	...
	if ( lpMsg->btFlag == 1 )
	{
		...
		gSendProto.DataSend(lpMsg->wRequestUserIndex, (LPBYTE)&pMsg, sizeof(pMsg));

		pMsg.btTargetUserIndexH = SET_NUMBERH(lpMsg->wRequestUserIndex);
		pMsg.btTargetUserIndexL = SET_NUMBERL(lpMsg->wRequestUserIndex);

		gSendProto.DataSend(lpMsg->wTargetUserIndex, (LPBYTE)&pMsg, sizeof(pMsg));
	...
	if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION )
	{
		...
		CLog.LogAdd("[U.System][Union][Join] ( Union Master Guild: %s ), (Union Member Guild: %s)",
	...
	else if ( lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL )
	{
		...
		CLog.LogAdd("[U.System][Rival][Join] ( %s ) vs ( %s )",
...
void DGUnionBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[GS][U.System][Union][BreakOff] Request Union Member Guild: %s)",
		lpReqGuild->Name);
	...
	CLog.LogAdd("[GS][U.System][Union][BreakOff] ( Union Master Guild: %s ), (Union Member Guild: %s)",
		lpTargetGuild->Name, lpReqGuild->Name);
...
void DGRelationShipAnsBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[U.System][Rival][BreakOff] (  %s ) vs ( %s )",
		lpReqGuild->Name, lpTargetGuild->Name);
...
void DGRelationShipNotificationRecv(EXSDHP_NOTIFICATION_RELATIONSHIP * lpMsg)
{
	if ( lpMsg->btUpdateFlag == EX_RESULT_FAIL_FOR_CASTLE )
	{
		...
		if ( lpGuildInfo )
		{
			...
			CLog.LogAdd("[U.System][Union][Auto BreakOff] ( Union Master Guild: %s ), (Union Member Guild: %s)",
...
void DGUnionListRecv(LPBYTE aRecv)
{
	...
	C2HeadSet((LPBYTE)lpMsg, 0xE9, lpMsg->btCount * sizeof(PMSG_UNIONLIST) + 0x10);	// #warning Check plus operation ( why 0x10 ??? )
	gSendProto.DataSend(lpRecvMsg->wRequestUserIndex, (LPBYTE)lpMsg, MAKE_NUMBERW(lpMsg->h.sizeH, lpMsg->h.sizeL));
}
...
void DGRelationShipAnsKickOutUnionMember(EXSDHP_KICKOUT_UNIONMEMBER_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[GS][U.System][Union][KickOut] ( Union Master Guild: %s ), (Union Member Guild: %s)",
...
void FriendListRequest(int aIndex)
{
	...
	{
		CLog.LogAdd("error-L3 : [%s][%s] FriendServer Online Request ", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}

	if (gObj[aIndex].m_FriendServerOnline == FRIEND_SERVER_STATE_OFFLINE )
	{
		CLog.LogAdd("error-L3 : [%s][%s] FriendServer Offline", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}
	...
	CLog.LogAdd("[%s] Friend List Request", gObj[aIndex].Name);
...
void FriendListResult(LPBYTE lpMsg)
{
	...
	gSendProto.DataSend(lpCount->Number, TmpSendBuf, pOfs);

	CLog.LogAdd("[%s] Friend List Count (%d) Send",
...
void WaitFriendListResult(FHP_WAITFRIENDLIST_COUNT * lpMsg)
{
	...
	{
		CLog.LogAdd("error-L2 : (%s) Index %s %d ",
	...
	gSendProto.DataSend(lpMsg->Number, (LPBYTE)&pMsg, sizeof(pMsg));

	CLog.LogAdd("[%s] WaitFriend List [%s]", gObj[lpMsg->Number].Name, Name.GetBuffer());
}
...
void FriendStateRecv(FHP_FRIEND_STATE * lpMsg)
{
	...
	if ( !gObjIsConnectedGP(lpMsg->Number, Name.GetBuffer()))
	{
		CLog.LogAdd("error-L2 : (%s) Index %s %d ", Name.GetBuffer(), __FILE__, __LINE__);
		return;
	}
	...
	CLog.LogAdd("[%s] Friend State (%d)", gObj[lpMsg->Number].Name, lpMsg->State);
}
...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend add request Name: [%s]", gObj[aIndex].Name,		szFriendName.GetBuffer());

	if ( !strcmp(gObj[aIndex].Name, szFriendName.GetBuffer()))
	{
		CLog.LogAdd("error-L3 : [%s] Friend add : Not self", gObj[aIndex].Name);
		...
		gSendProto.DataSend(aIndex, (LPBYTE)&pResult, sizeof(pResult));
	}
	else
	{
		wsExDbCli.DataSend((PCHAR)&pMsg, pMsg.h.size);
...
void FriendAddResult(FHP_FRIEND_ADD_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[%s] Friend add result : (%d) friend Name : (%s)",
...
void WaitFriendAddRequest(PMSG_FRIEND_ADD_SIN_RESULT * lpMsg, int aIndex)
{
...

	{
		...
		CLog.LogAdd("[%s] WaitFriend add request [%s]", gObj[aIndex].Name, fname.GetBuffer());
	}
}
...
void WaitFriendAddResult(FHP_WAITFRIEND_ADD_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[%s] WaitFriend add result (%d) [%s]",
		Name.GetBuffer(), lpMsg->Result, szFriendName.GetBuffer());
...
void FriendDelRequest(PMSG_FRIEND_DEL_REQ * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend del request [%s]", gObj[aIndex].Name, Name.GetBuffer());
}
...
void FriendDelResult(FHP_FRIEND_DEL_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[%s] Friend del result (%d) [%s]",
...
void FriendChatRoomCreateReq(PMSG_FRIEND_ROOMCREATE_REQ * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Chatroom create request [%s]", gObj[aIndex].Name, szName.GetBuffer());
}
...
void FriendChatRoomCreateResult(FHP_FRIEND_CHATROOM_CREATE_RESULT * lpMsg)
{
	...
	if ( !gObjIsConnectedGP(lpMsg->Number, Name.GetBuffer()))
	{
		CLog.LogAdd("error-L2 : Index %s %d (%s)", __FILE__, __LINE__, Name.GetBuffer());
		return;
	}
	...
	CLog.LogAdd("[%s] Chatroom create result (%d) ticket:(%d)",
...
void FriendMemoSend(PMSG_FRIEND_MEMO * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend mail send %s (Size:%d)", gObj[aIndex].Name, Name.GetBuffer(), bsize);
}
...
void MngFriendMemoSend(PMSG_JG_MEMO_SEND * lpMsg)
{
	...
	CLog.LogAdd("JoinServer mail send %s (Size:%d)", Name.GetBuffer(), bsize);
}
...
BOOL WithdrawUserMoney(LPSTR Type, LPOBJ lpObj, int Withdraw_Money)
{
	...
	if ( (lpObj->Money - Withdraw_Money) >= 0 )
	{
		...
		CLog.LogAdd("[%s][%s] (%s) Pay Money(In Inventory) : %d - %d = %d",
...
void FriendMemoSendResult(FHP_FRIEND_MEMO_SEND_RESULT * lpMsg)
{
	...
	{
		CLog.LogAdd("JoinServer Send Mail result : (%d)", lpMsg->Result);
		return;
	}
	...
	CLog.LogAdd("[%s] Friend mail Send result : (%d) guid:(%d)",
...
void FriendMemoListReq(int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend mail list request", gObj[aIndex].Name);
}
...
void FriendMemoList(FHP_FRIEND_MEMO_LIST * lpMsg)
{
	...
	CLog.LogAdd("[%s] Friend Mail list : (%d:%10s:%s:%s)",
...
void FriendMemoReadReq(PMSG_FRIEND_READ_MEMO_REQ * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend Mail read Index:%d", 
...
void FriendMemoRead(FHP_FRIEND_MEMO_RECV * lpMsg)
{
	...
	{
		CLog.LogAdd("error-L2 : Friend Memo Max %s %d", __FILE__, __LINE__);
		return;
	}
	...
	gSendProto.DataSend(lpMsg->Number, (LPBYTE)&pMsg, nsize);

	CLog.LogAdd("[%s] Friend Mail read (%d)",
...
void FriendMemoDelReq(PMSG_FRIEND_MEMO_DEL_REQ * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend mail delete request Index:(%d)",
		gObj[aIndex].Name, lpMsg->MemoIndex);
}
...
void FriendMemoDelResult(FHP_FRIEND_MEMO_DEL_RESULT * lpMsg)
{
	...
	CLog.LogAdd("[%s] Friend mail delete request (%d) index:(%d)",
...
void FriendRoomInvitationReq(PMSG_ROOM_INVITATION * lpMsg, int aIndex)
{
	...
	CLog.LogAdd("[%s] Friend Invistation request room:%d winguid:%d",
...
void FriendRoomInvitationRecv(FHP_FRIEND_INVITATION_RET* lpMsg)
{
	...
	CLog.LogAdd("[%s] Friend Invitation result :%d",
