...
void CCastleDeepEvent::CheckSync()
{
	...
	{
		CLog.LogAddC(2,"[CastleDeep Event] No Schedule Time Data");
	...
	CLog.LogAdd("[CastleDeep Event] Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
...
void CCastleDeepEvent::AddMonster(int iAssultType,int iGroup)
{
	...
	{
		CASTLEDEEP_MONSTERINFO stMonsterInfo2 = *it2;
		CASTLEDEEP_MONSTERINFO * stMonsterInfo = &stMonsterInfo2;
...
using namespace std;
	...
	if(SMDFile == NULL)
	{
		CLog.MsgBox("[CastleDeep Event] Info file Load Fail [%s]",lpszFileName);
		return false;
	}
...
void CCastleDeepEvent::ProcState_Closed()
{
	...
	if(iTICK_MSEC >= 1000)
	{
		...
		CLog.LogAddC(3,"[CastleDeep Event] ProcState_Closed()	LEFT-MSEC:%d",m_iTIME_MSEC_REMAIN);
	}
	...
	if(m_iTIME_MSEC_REMAIN <= 0)
	{
		...
		CLog.LogAdd("[CastleDeep Event] - Event Started");
	}
}
...
using namespace std;
	...
	if(iTICK_MSEC >= 1000)
	{
		...
		CLog.LogAddC(3,"[CastleDeep Event] ProcState_Playing()	LEFT-MSEC:%d",m_iTIME_MSEC_REMAIN);
	}
	...
	if(m_mapRunCheck.size() <= 0)
	{
		...
		CLog.LogAdd("[CastleDeep Event] - Event Ended");
	}
}
...
void CCastleDeepEvent::StartEvent()
{
	...
	{
		CLog.LogAdd("[CastleDeep Event] Start Fail - No Assult Data");
		SetState(CD_STATE_NONE);
		return;
	}
	...
	if(bDoStart == FALSE)
	{
		CLog.LogAdd("[CastleDeep Event] Start Fail - bDoStart == FALSE");
		SetState(CD_STATE_NONE);
		return;
	}
	...
	for(vector<CASTLEDEEP_SPAWNTIME>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
	{
		...
		m_mapRunCheck.insert( pair<int,int>(it2->m_iMonsterGroup,it2->m_iMonsterSpawnMinute) );
	}
}
