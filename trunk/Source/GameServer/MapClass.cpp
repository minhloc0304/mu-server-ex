...
{
	if ( MapNumber < 0 || MapNumber > MAX_NUMBER_MAP-1 )
	{
		return FALSE;
	}
	return TRUE;
...
void MapClass::GetLevelPos(short level, short& ox, short& oy)
{
	...
	CLog.LogAdd( lMsg.Get( MSGGET(1, 204 )) , __FILE__, __LINE__);
...
// -----------------------------------------------------------------------------------------------------------------------
void MapClass::LoadMapAttr(const char * filename, int MapNumber)
{
	if (this->AttrLoad(filename) == FALSE )
	{
		CLog.MsgBox(lMsg.Get( MSGGET ( 2, 52 )));
	}
...
// -----------------------------------------------------------------------------------------------------------------------
int MapClass::MonsterItemDrop(int type, int level, float dur,  int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption,  int aIndex, DWORD number, BYTE ItemEffectEx, BYTE * SocketOption)
{
	int count;
	int counttot=0;
	...
	while ( true )
	{
		...
		{
			this->m_cItem[count].CreateItem(type, level, x, y, dur, Option1, Option2, Option3, NOption, SOption, number, ItemEffectEx, SocketOption);
	...
	CLog.LogAdd(lMsg.Get(MSGGET(1, 205)), __FILE__, __LINE__ );

	return -1;
}
// -----------------------------------------------------------------------------------------------------------------------
BOOL MapClass::ItemDrop(int type, int level, float dur,  int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption, DWORD number, int aIndex, int PetLevel, int PetExp, BYTE ItemEffectEx, BYTE * SocketOption)
{
	int count;
	int counttot = 0;
	BYTE attr = this->GetAttr(x, y);
	...
	while ( true )
	{
		if ( this->m_cItem[count].IsItem() == FALSE )
		{
			this->m_cItem[count].DropCreateItem(type, level, x, y, dur, Option1, Option2, Option3, NOption, SOption, number, PetLevel, PetExp, ItemEffectEx,SocketOption);
			this->m_cItem[count].m_UserIndex = aIndex;
			this->m_ItemCount++;
			...
			if ( gUser.BloodCastleMapRange(this->thisMapNumber) == true )
			{
				if ( type == ITEMGET(13,19) )
				{
					if ( ((level<0)?FALSE:(level>2)?FALSE:TRUE) != FALSE )
					{
						if ( g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_nBC_QUESTITEM_SERIAL != -1 )
						{
							if ( number == g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_nBC_QUESTITEM_SERIAL )
							{
								if ( g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_iBC_QUEST_ITEM_USER_INDEX != -1 )
								{
									CLog.LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Dropped Angel King's Weapon (%d)",
										g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)+1, gObj[g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_iBC_QUEST_ITEM_USER_INDEX ].AccountID,
										gObj[g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_iBC_QUEST_ITEM_USER_INDEX].Name,
										g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_btBC_QUEST_ITEM_NUMBER);
									
									g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_iBC_QUEST_ITEM_USER_INDEX = -1;
									g_BloodCastle.m_BridgeData[g_BloodCastle.GetBloodCastleIndex(this->thisMapNumber)].m_btBC_QUEST_ITEM_NUMBER = 0;
...
BOOL MapClass::ItemGive(int aIndex, int item_num, bool bFailNotSend)
{
	...
	{
		//CLog.LogAdd(lMsg.Get(MSGGET(1, 206)), __FILE__, __LINE__);
		return FALSE;
	}

	if ( gObj[aIndex].MapNumber != this->thisMapNumber )
	{
		//CLog.LogAdd(lMsg.Get(MSGGET(1, 207)), __FILE__, __LINE__, gObj[aIndex].MapNumber, gObj[aIndex].Name);
		return FALSE;
	}

	if ( this->m_cItem[item_num].IsItem() == FALSE )
	{
		//CLog.LogAdd(lMsg.Get(MSGGET(1, 208)), __FILE__, __LINE__,gObj[aIndex].Name);	// Apply Deathway Fix herw
		return FALSE;
	}
...
void MapClass::StateSetDestroy()
{
	...
	for (int n=0;n<MAX_MAPITEM;n++)
	{
		...
		if ( this->m_cItem[n].IsItem() != FALSE )
		{
			...
			{
				if ( CurTime > (int)this->m_cItem[n].m_Time )
				{
					CLog.LogAdd(lMsg.Get(MSGGET(4, 76)), this->m_cItem[n].GetName(), this->m_cItem[n].m_Type, this->m_cItem[n].m_Level, this->m_cItem[n].m_Special[0] , this->m_cItem[n].m_Number);
...
// -----------------------------------------------------------------------------------------------------------------------
BOOL MapClass::AttrLoad(const char* filename)
{
	FILE * fp;
	unsigned char head;
	...
	{
		CLog.MsgBox(lMsg.Get( MSGGET(2, 51)));
		fclose(fp);
		return false;
	}
