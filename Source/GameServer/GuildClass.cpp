...
// Description: MU Guild Class Structures & Functions					//
...
BOOL CGuildClass::SetGuildMemberStatus(char* szGuildName, char* szMemberName, int iGuildStatus)
{
	...
	if ( pNode->Index[iKey] > 0 )
	{
		...
		{
			CLog.LogAdd("@SetGuildMemberStatus() error-L2 : Index %s %d", __FILE__, __LINE__);
			return false;
		}
	...
	for ( int n=0;n<MAX_USER_GUILD;n++)
	{
		if ( pNode->Use[n] == TRUE )
		{
			if ( pNode->Index[n] > 0 )
			{
				...
				{
					CLog.LogAdd("SetGuildType() error-L2 : Index %s %d", __FILE__, __LINE__);
					continue;
				}
