...
void CMonsterSetBase::LoadSetBase(char * filename)
{
	...
	if ( SMDFile == NULL )
	{
		CLog.MsgBox("file open error %s", filename);
		return;
	}
				...
				if ( Type == 1 || Type == 3 )
				{
					...
					if ( rcount > 1 )
					{
						for ( int k = 0; k<rcount-1 ; k++ )
						{
							...
							{
								CLog.MsgBox("Monster attribute max over %d (%s %d)",this->m_Count,__FILE__, __LINE__);
								fclose(SMDFile);
								return;
							}
				...
				if ( Type == 1 || Type == 3 )
				{
					...
					if ( rcount > 1 )
					{
						for ( int k = 0; k<rcount-1 ; k++ )
						{
							...
							if ( this->m_Count > OBJ_MAXMONSTER-1 )
							{
								CLog.MsgBox("Monster attribute max over %s %d", __FILE__, __LINE__);
								return;
							}
